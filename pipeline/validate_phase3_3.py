"""Strict, exported-CAD validation for Phase 3.3."""
...
from phase3_objective import bbox, boundary_f1, detail_regions, evaluate_regions, read_component_masks
from reconstruct_phase3 import CAMERAS
from refine_phase3_1 import _camera, _jewelry_transform, _render
from refine_phase3_3 import VIEW_WEIGHTS, VISIBILITY, VIEWS, _camera_with_azimuth, _perspective_transform, _render_perspective
...
def _fit_angled(mesh, target, frame, center_tilt=32, center_roll=22, center_azimuth=0, center_distance=60.0):
    ...
    distances = sorted({max(24.0, center_distance * factor) for factor in (0.86, 1.0, 1.18)})
    for tilt in range(center_tilt - 4, center_tilt + 5, 2):
        for roll in range(center_roll - 4, center_roll + 5, 2):
...
def validate(input_dir: Path, masks_dir: Path, previous_dir: Path, phase_dir: Path) -> dict:
    build = json.loads((phase_dir / "phase3_3_build.json").read_text(encoding="utf-8"))
    fit = json.loads((phase_dir / "phase3_3_fit.json").read_text(encoding="utf-8"))
    ...
    meshes = {
        "jewelry": trimesh.load(phase_dir / "ring01_phase3_3.stl", force="mesh"),
        "shank": trimesh.load(phase_dir / "ring01_phase3_3_shank.stl", force="mesh"),
        "stone_amodal": trimesh.load(phase_dir / "ring01_phase3_3_stone.stl", force="mesh"),
        "setting": trimesh.load(phase_dir / "ring01_phase3_3_setting.stl", force="mesh"),
        "prongs": trimesh.load(phase_dir / "ring01_phase3_3_prongs.stl", force="mesh"),
    }
    previous = trimesh.load(previous_dir / "ring01_phase3_2.stl", force="mesh")
    ...
    camera_fit["search"] = "cached local tilt/roll/azimuth +/-4 degrees and three distances around proxy fit"
    ...
    for view in VIEWS:
        ...
        old = _overlay(image, target, previous_mask, "PHASE 3.2 ERROR")
        ...
        path = phase_dir / "comparisons" / f"ring01_{view}_reference_vs_phase3_3.png"
    ...
    preview_path = phase_dir / "previews" / "ring01_phase3_3_clean_preview.png"
    ...
    report = {
        "stage": "phase3_3_strict_exported_cad_validation", "checkpoint_valid": all(v for k, v in topology_checks.items() if k != "stl_watertight"),
        ...
        "principal_view_floor": round(principal_floor, 6), "camera_fit": {"angled": camera_fit},
        "topology_checks": topology_checks, "exit_checks": exit_checks, "views": views,
        "comparisons": comparisons, "clean_preview": str(preview_path),
        "uncertainty": {"hidden_geometry": "inferred", "physical_scale": "uncalibrated", "reference_masks": "machine-generated, not human ground truth",
    ...
    (phase_dir / "phase3_3_validation.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    return report
...
def main() -> None:
    ...
    parser.add_argument("--previous-dir", type=Path, default=Path("data/ring01_phase3_2"))
    parser.add_argument("--phase-dir", type=Path, default=Path("data/ring01_phase3_3"))
